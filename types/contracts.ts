...
export type TokenContract = Contract & {
    ...
    ft_transfer: (receiver_id: string, amount: string, memo: any) => Promise<void>;
...
export type PNFTContract = Contract & {
    ...
    repost_details: (arg: { user_id: string; repost_id: number; }) => Promise<any>;
...
export type ProfileContract = Contract & {
    ...
    swap: (arg: { amount: string; min_expected: string; }, gas: string,) => Promise<void>;
    list_post_for_sale: (arg: {
        post_id: number,
        price: string;
    }, gas: string,
    ) => Promise<boolean>;
    transfer_ownership: (arg: {
        current_owner: string,
        new_owner: string,
        post_id: number;
    }, gas: string,
    ) => Promise<boolean>;
    buy_post: (arg: { post_id: number }, gas: string, deposit: string,) => Promise<boolean>;
    is_username_available: (username: string) => Promise<boolean>;
    has_registered: (user_id: string) => Promise<boolean>;
    profile_by_id: (arg: {
        user_id: string;
...
export type DexContract = Contract & {
    ...
    get_user_share: (arg: { account_id: string; pool_id: number; }) => Promise<number>;
