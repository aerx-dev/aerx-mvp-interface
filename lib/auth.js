...
export async function checkProfile(nearState) {
    ...
    if (nearState.accountId && nearState.pnftContract) {
        const profileNft = await nearState.pnftContract.nft_tokens_for_owner({ account_id: nearState.accountId })
        // If the NFT has Media, grab it!
        if (profileNft.lenght > 0) {
...
async function loadCNFTContract(nearState) {
    ...
    var responseFeed = await cnftContract.nft_tokens({}).catch((err) => { console.error(err); }) || [];
    if (responseFeed) {
