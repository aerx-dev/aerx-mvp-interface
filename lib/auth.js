...
export async function checkToken(nearState) {
    // checks token is initialised and user is connected
    if (nearState.tokenContract && nearState.accountId) {
        console.log("Checking Token...", nearState.tokenContract);
        const balance = await nearState.tokenContract?.ft_balance_of({
            account_id: nearState.accountId,
        });
        console.log(balance);
    }
