...
const TOKEN_CONTRACT_NAME = "ttest.mohzcrea8me.testnet"; //to load aex_token contract
const PROFILES_CONTRACT_NAME = "ttest.mohzcrea8me.testnet"; //to load aex_token contract
...
export async function initNearConnection(nearState) {
    ...
    await loadTokenContracts(nearState, walletConnection.account());
...
async function loadTokenContracts(nearState, account) {
    window.tokenContracts = await new Contract(account, PROFILES_CONTRACT_NAME, {
        // View methods are read only. They don't modify the state, but usually return some value.
        changeMethods: [
            "swap"
        ],
    ...
    nearState.setProfileTokenContract(window.tokenContracts);
}
