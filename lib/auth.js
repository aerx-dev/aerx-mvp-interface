...
export async function initNearConnection(nearState) {
    ...
    loadTokenContract(nearState, walletConnection.account())
        .then(() => loadPNFTContract(nearState))
        .then(() => loadCNFTContract(nearState));
    // here ! since none of the conditionals runs synchronously,
    // it adds all of them to stack at once
    // but the checkprofile is the first to leave the callstack
    // because the loadpnft may not be added to state before checkProfile is run
    // so checkprofile runs once without setting to state
    // after the cnft and pnft async task completes check profile has already ran
    // and profile is not set to state.
}
