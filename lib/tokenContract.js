...
export async function getBalance(nearState) {
    if (!nearState.tokenContract) {
        await nearState.tokenContract;
    }
    const res = await nearState.tokenContract.ft_balance_of({
        account_id: state.accountId,
    });
    const blnc = new Big(res);
    const formatted = blnc.div("10e23").toFixed(1);
    if (formatted !== nearState.aexBalance) {
        nearState.setAexBalance(formatted);
    }
