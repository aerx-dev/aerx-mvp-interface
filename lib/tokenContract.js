...
export async function getBalance(state) {
    if (state.tokenContract) {
        ...
        if(isUserRegistered) {
            ...
            console.log("user is not registered will do nothing");
        }
        
    }else{
        console.log("Error! token contract doesn't have a state yet verify contract has beign initialised and try again");
...
export async function fetchpostsData(state) {
    if (state.pnftContract) {
        ...
        console.log("Error! pnft contract doesn't have a state yet verify contract has beign initialised and try again");
        //todo: throw an error alert
    }
}
